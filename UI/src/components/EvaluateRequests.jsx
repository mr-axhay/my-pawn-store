...
const EvaluateRequests = () => {
  ...
  const fetchRequests = async () => {
    try {
      const res = await axios.get(
        "https://my-pawn-store.onrender.com/api/evaluate",
      );
  ...
  const handleApprove = async (id) => {
    await axios.put(
      `https://my-pawn-store.onrender.com/api/evaluate/${id}/approve`,
    );
    fetchRequests();
  };
  ...
  const handleReject = async (id) => {
    await axios.put(
      `https://my-pawn-store.onrender.com/api/evaluate/${id}/reject`,
    );
    fetchRequests();
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-gray-900 to-gray-800 p-6 text-white">
      <h1 className="text-3xl font-bold mb-6">Evaluate Requests</h1>
      {requests.length ? (
